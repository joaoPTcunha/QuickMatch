...
<body class="flex flex-col min-h-screen bg-gray-100">
    <div class="flex flex-grow">
        <!-- Barra lateral para a lista de usuários -->
        <div class="w-1/4 bg-gray-50 p-4 border-r flex flex-col">
            ...
            <h2 class="text-xl font-semibold mb-4">Utilizadores</h2>
            <ul class="space-y-2">
                @foreach($users as $user)
                    <li class="flex items-center p-2 rounded-lg hover:bg-gray-200 cursor-pointer" onclick="loadMessages({{ $user->id }})">
                        <img src="{{ $user->profile_picture ? asset('storage/' . $user->profile_picture) : 'https://ui-avatars.com/api/?name=' . urlencode($user->name) }}" alt="Imagem de perfil" class="w-10 h-10 rounded-full mr-3">
